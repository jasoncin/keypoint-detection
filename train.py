...
from KeypointDataset import DocumentTrainDataset
...
def train(prepared_train_labels, train_images_folder, num_refinement_stages, base_lr, batch_size, batches_per_iter,
          ...
          val_labels, val_images_folder, val_output_name, checkpoint_after, val_after, epochs):
    net = PoseEstimationWithMobileNet(num_refinement_stages)

    stride = 8
    sigma = 7
    path_thickness = 1
    dataset = DocumentTrainDataset(prepared_train_labels, train_images_folder,
            ...
            if num_iter % log_after == 0:
                ...
                for loss_idx in range(len(total_losses) // 2):
                    ...
                    print('\n'.join(['stage{}_pafs_loss:     {}', 'stage{}_heatmaps_loss: {}']).format(
                        loss_idx + 1, total_losses[loss_idx * 2 + 1] / log_after,
                        loss_idx + 1, total_losses[loss_idx * 2] / log_after))
                    if min_pafs_loss > total_losses[loss_idx * 2] / log_after:
                        min_pafs_loss = total_losses[loss_idx * 2] / log_after
                        snapshot_name = '{}/best_loss.pth'.format(checkpoints_folder)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--prepared-train-labels', type=str,  default=r'/opt/ml/input/data/train/dataset/test_data/prepared_train_annotation.pkl',
                        help='path to the file with prepared annotations')
    parser.add_argument('--train-images-folder', type=str,  default=r'/opt/ml/input/data/train/dataset/test_data', help='path to COCO train images folder')
    ...
    parser.add_argument('--checkpoint-path', type=str, default= "/opt/ml/input/data/train/checkpoints/mobilenet_sgd_68.848.pth",
    ...
    parser.add_argument('--experiment-name', type=str, default='TestCheckpoint',
                        help='experiment name to create folder for checkpoints')
    parser.add_argument('--log-after', type=int, default=20, help='number of iterations to print train loss')
    parser.add_argument('--val-labels', type=str, default='val_subset.json', help='path to json with keypoints val labels')
    parser.add_argument('--val-images-folder', type=str,default = r'data\\COCO_HOME\\val2017', help='path to COCO val images folder')
    ...
    parser.add_argument('--checkpoint-after', type=int, default=500,
                        help='number of iterations to save checkpoint')
    parser.add_argument('--val-after', type=int, default=400,
    ...
    train(args.prepared_train_labels, args.train_images_folder, args.num_refinement_stages, args.base_lr, args.batch_size,
          ...
          args.checkpoint_after, args.val_after, args.epochs)
