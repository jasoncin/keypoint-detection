...
class ImageReader(object):
    ...
    def __next__(self):
        ...
        file_name = self.file_names[self.idx].split('/')[-1]
        self.idx = self.idx + 1
        return padding_warped_image, file_name
...
def run_demo(net, image_provider, height_size, cpu, track_ids):
    ...
    previous_poses = []
    idx = 0
    for tmp_img in image_provider:
        start_time = time.time()
        img, img_name = tmp_img
        orig_img = img.copy()
        scale = 640 / orig_img.shape[0]
        ...
        try:
            for keypoint in all_keypoints:
                cv2.circle(img, (int(keypoint[0]), int(keypoint[1])), 2, (255, 0, 0), thickness=2, lineType=8, shift=0)
            idx += 1
            cv2.imwrite("visualize_kps/{}".format(img_name), img)
            continue
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint-path', type=str, default='weights/best_loss_multi.pth',
                        help='path to the checkpoint')
    parser.add_argument('--height-size', type=int, default=640, help='network input layer height size')
    parser.add_argument('--video', type=str, default='', help='path to video file or camera id')
    parser.add_argument('--images', nargs='+',
                        default=[r'D:/Coding/DocHomographyGenerator/res/output/train_1/Image_39.png'],
    ...
    list_file = []
    dir_test = r"data/sample/images"
    for file in os.listdir(dir_test):
        if file.endswith("JPG") or file.endswith("jpg") or file.endswith("png"):
            list_file.append(dir_test + "/" + file)
    ...
    run_demo(net, frame_provider, args.height_size, True, args.track_ids)
